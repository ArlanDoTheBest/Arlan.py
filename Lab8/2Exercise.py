...
def game_loop():
    ...
    while not game_exit:
        ...
        # If snake eats food
        if snake_x == food_x and snake_y == food_y and food_color == RED:
            score += 1
            snake_length += 1
            food_x, food_y, food_color = generate_food()
            food_timer = time.time() + 10  # Reset food timer
        elif snake_x == food_x and snake_y == food_y and food_color == YELLOW:
            score += 2
            snake_length += 1
            food_x, food_y, food_color = generate_food()
        ...
        pygame.draw.circle(gameDisplay, food_color, (food_x +BLOCK_SIZE//2 , food_y + BLOCK_SIZE // 2) , BLOCK_SIZE//2)
